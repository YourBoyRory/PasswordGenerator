...
class Config:
    ...
    def get_config(self):
        ...
        if os.name == "posix":  # If we are on Linux or MacOS
            self.systemwide_config = Path('/ect/PasswordGenerator/') / self.config_path
            self.override_path = Path('/ect/PasswordGenerator/override.in')
        else:                   # If we are on Windows
            self.systemwide_config = Path('C:\ProgramData\PasswordGenerator\\') / self.config_path
            self.override_path = Path('C:\ProgramData\PasswordGenerator\\override.ini')
    ...
    def read_config(self):
        ...
        try:
            ...
            self.parser.read([self.systemwide_config, self.user_config2, self.user_config1, self.config_path, self.override_path])
            self.store_values()
        except:
            try:
                self.parser['General'] = { }
                self.parser['Wordlist'] = { }
                self.parser.read(self.override_path)
                self.store_values()
                print("[ \033[31mERROR\033[0m ] Malformed config.ini!")
                print("[ \033[33mWARN\033[0m ] Falling back to default values.")
            except:
                input("[ \033[31m\033[1mFATAL\033[0m ] Malformed override.ini!")
                exit()
    ...
    def save_config(self):
        ...
        self.parser['General'] = {
            ...
            'use_system_config_first': self.value['use_system_config_first'],
        }
        self.parser['Wordlist'] = {
